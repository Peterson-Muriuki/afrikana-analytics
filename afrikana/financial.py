...
SCENARIO_OVERRIDES = {
    "Base": {},
    "Bull": {
        "swap_price_usd": 3.20, "swaps_per_station_day": 25.0,
        "opex_per_station_month": 200.0, "station_growth_monthly": 0.06,
    },
    "Bear": {
        "swap_price_usd": 1.80, "swaps_per_station_day": 10.0,
        "opex_per_station_month": 260.0, "station_growth_monthly": 0.01,
    },
}
...
class FinancialModel:
    ...
    **kwargs
    ...
    def unit_economics(self) -> dict:
        ...
        ebitda         = gross_profit - p.opex_per_station_month \
                         - p.staff_cost_per_station - logistics_cost
        ebit           = ebitda - capex_monthly
        nopat          = ebit * (1 - p.tax_rate)
        var_cost       = p.swap_price_usd * (1 - p.gross_margin_pct) \
        ...
        return {
            ...
            "contribution_margin_pct":    round(contrib / p.swap_price_usd * 100, 1) if p.swap_price_usd else 0,
    ...
    def pl_projection(self) -> pd.DataFrame:
        ...
        for m in range(1, p.projection_months + 1):
            ...
            rows.append({
                "month": m, "n_stations": round(n, 1),
                "revenue": round(revenue, 0), "cogs": round(cogs, 0),
                "gross_profit": round(gross_profit, 0),
                "opex": round(opex + logistics, 0),
                "ebitda": round(ebitda, 0), "depreciation": round(depreciation, 0),
                "ebit": round(ebit, 0), "tax": round(tax, 0),
        ...
        for _ in range(n_sims):
            try:
                sim = FinancialModel(
                    **{
                        **self._config_dict(),
                        "swap_price_usd":         max(0.5, rng.normal(p.swap_price_usd,       p.swap_price_usd * 0.15)),
                        "swaps_per_station_day":  max(1.0, rng.normal(p.swaps_per_station_day, p.swaps_per_station_day * 0.20)),
                        "opex_per_station_month": max(50,  rng.normal(p.opex_per_station_month, p.opex_per_station_month * 0.12)),
