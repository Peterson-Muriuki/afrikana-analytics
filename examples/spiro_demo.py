...
print(
    model.scenarios()[
        ["Scenario", "Revenue/Stn ($)", "EBITDA/Stn ($)", "NPV ($)", "Payback (months)"]
...
print(
    f"  NPV P10/P50/P90   : ${mc['npv_p10']:,.0f} / ${mc['npv_p50']:,.0f} / ${mc['npv_p90']:,.0f}"
)
...
print(
    top5[["candidate_id", "deployment_score", "priority", "nearest_station_km"]].to_string(
        index=False
    )
)
...
print(
    f"Peak hour          : {peaks.loc[peaks['swaps'].idxmax(), 'hour']}:00 "
